 ...
 * ```
 *
 * For a smaller bundle, import the flat functions from `jira.js/cloud`, `jira.js/agile`, `jira.js/serviceDesk`,
 * `jira.js/server`, `jira.js/serviceDeskServer` or `jira.js/assetsServer` and drive them with that same client.
 *
 ...
 * `createUserProvisioningClient` for SCIM. All three answer on `https://api.atlassian.com` rather than on a site.
 * `createAdminClient` and `createUserManagementClient` take an organization API key; `createUserProvisioningClient`
 * takes the API key of the SCIM directory, from its identity provider configuration.
 *
...
export { createServerClient, type ServerClient, type ServerClientConfig } from './server/createServerClient.js';

export {
  createServiceDeskServerClient,
  type ServiceDeskServerClient,
  type ServiceDeskServerClientConfig,
} from './serviceDeskServer/createServiceDeskServerClient.js';

export {
  createAssetsServerClient,
  type AssetsServerClient,
  type AssetsServerClientConfig,
} from './assetsServer/createAssetsServerClient.js';
