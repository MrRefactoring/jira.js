...
export class ServiceDesk {
  ...
  /**
   * This method returns a service desk. Use this method to get service desk details whenever your application
   * component is passed a service desk ID but needs to display other service desk details.
   *
   * **[Permissions](#permissions) required**: Permission to access the Service Desk. For example, being the Service
   * Desk's Administrator or one of its Agents or Users.
   */
   ...
   *
   * Because this method accepts multipart/form-data, it has XSRF protection on it. This means you must submit a
   * header of X-Atlassian-Token: no-check with the request or it will be blocked.
   *
   ...
   * customers' `displayName`, `name`, or `email`. For example, searching for "John", "Jo", "Smi", or "Smith" will
  ...
  /**
   * Adds one or more customers to a service desk. If any of the passed customers are associated with the service
   * desk, no changes will be made for those customers and the resource returns a 204 success code.
   *
  ...
  /**
   * This method removes one or more customers from a service desk. The service desk must have closed access. If any
   * of the passed customers are not associated with the service desk, no changes will be made for those customers and
   * the resource returns a 204 success code.
   *
  ...
  async getQueues<T = Models.PagedQueue>(
    parameters: Parameters.GetQueues,
    callback?: Callback<T>,
  ): Promise<void | T> {
    const config: RequestConfig = {
  ...
  /**
   * This method returns a specific queues in a service desk. To include a customer request count for the queue (in
   * the `issueCount` field) in the response, set the query parameter `includeCount` to true (its default is false).
   *
   * **[Permissions](#permissions) required**: service desk's Agent.
   */
   ...
   * - `searchQuery` which is matched against request types' `name` or `description`. For example, the strings
   *   "Install", "Inst", "Equi", or "Equipment" will match a request type with the *name* "Equipment Installation Request".
   *
   * **[Permissions](#permissions) required**: Permission to access the service desk.
   */
  ...
  /**
   * This method enables a customer request type to be added to a service desk based on an issue type. Note that not
   * all customer request type fields can be specified in the request and these fields are given the following default values:
   *
   ...
   * - Request type status mapping is left empty, so the request type has no custom status mapping but inherits the
   *   status map from the issue type upon which it is based.
   ...
   *
   * **[Permissions](#permissions) required**: Permission to view the Service Desk. However, hidden fields would be
   * visible to only Service desk's Administrator.
   */
   ...
   *
   * Properties for a Request Type in next-gen are stored as Issue Type properties and therefore can also be deleted
   * by calling the Jira Cloud Platform [Delete issue type
