export interface GetCustomerRequestByIdOrKey {
   ...
   * status transitions, in chronological order. `attachment` returns the attachments. `action` returns the actions
   * that the user can or cannot perform. `comment` returns the comments. `comment.attachment` returns the attachment
   * details for each comment. `comment.renderedBody` (Experimental) return the rendered body in HTML format (in
