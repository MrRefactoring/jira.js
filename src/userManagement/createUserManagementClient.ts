...
export type UserManagementClientConfig = Omit<ClientConfig, 'auth' | 'host'> & {
   ...
   *
   * A key created with scopes is refused — measured: every operation answers 403 naming a `manage:org` scope the key
   * creation flow does not offer. Nothing else reaches this API either: a site API token answers 401, and so does OAuth
   * 2.0 (3LO), whose scopes are granted by a user for a site rather than by an organization for itself.
   */
  auth: AuthBearer;
