...
import { PageIssueTypeJsonSchema } from '../models/pageIssueTypeJson';
...
export async function getPaginatedIssueTypes(
  ...
  options?: RequestOptions,
): Promise<Page<IssueTypeJson>> {
  const config: SendRequestOptions<Page<IssueTypeJson>> = {
    ...
    schema: PageIssueTypeJsonSchema,
    signal: options?.signal,
  };
