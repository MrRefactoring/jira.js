...
export function bodyToFetchBody(body: unknown, contentType?: string): BodyInit | AsyncIterable<unknown> {
  ...
  // A declared media type means the endpoint reads the body as text rather than as JSON, so a string goes out as it
  // is. Without this a `text/plain` body arrived quoted, which reads as a different value or as no value at all.
