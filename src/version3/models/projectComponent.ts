...
export interface ProjectComponent {
   ...
   * `realAssigneeType` for details on how the type of the user, and hence the user, assigned to issues is determined.
   * Can take the following values:
   *
   * `PROJECT_LEAD` the assignee to any issues created with this component is nominally the lead for the project the
   * component is in. `COMPONENT_LEAD` the assignee to any issues created with this component is nominally the lead
   * for the component. `UNASSIGNED` an assignee is not set for issues created with this component. `PROJECT_DEFAULT`
   * the assignee to any issues created with this component is nominally the default assignee for the project that the
   * component is in.
   *
  ...
  id?: string;
  /**
   * Whether a user is associated with `assigneeType`. For example, if the `assigneeType` is set to `COMPONENT_LEAD`
   * but the component lead is not set, then `false` is returned.
   */
   ...
   * from the `assigneeType`. For example, `assigneeType` is set to `COMPONENT_LEAD` but no component lead is set.
   ...
   * lead has permission to be assigned issues in the project that the component is in. `UNASSIGNED` when
   * `assigneeType` is `UNASSIGNED` and Jira is configured to allow unassigned issues. `PROJECT_DEFAULT` when none of
   * the preceding cases are true.
   */
