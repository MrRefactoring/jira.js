...
/**
 * Returns all issues from the board's backlog, for the given board ID. Result pagination is token based, using
 * `nextPageToken` and `maxResults`. This only includes issues that the user has permission to view. Note, if the user
 * does not have permission to view the board, no issues will be returned at all. The backlog contains incomplete issues
...
/**
 * Returns the approximate count of all issues from the board's backlog, for the given board ID. This is equivalent to
 * counting the issues on all pages returned by [Get issues for backlog
...
/**
 * Returns all issues that do not belong to any epic on a board, for a given board ID. Result pagination is token based,
 * using `nextPageToken` and `maxResults`. This only includes issues that the user has permission to view. Note, if the
 * user does not have permission to view the board, no issues will be returned at all. Issues returned from this
...
/**
 * Returns all issues that belong to an epic on the board, for the given epic ID and the board ID. Result pagination is
 * token based, using `nextPageToken` and `maxResults`. This only includes issues that the user has permission to view.
...
/**
 * Returns all issues from a board, for a given board ID. Result pagination is token based, using `nextPageToken` and
 * `maxResults`. This only includes issues that the user has permission to view. Note, if the user does not have
 * permission to view the board, no issues will be returned at all. An issue belongs to the board if its status is
 * mapped to the board's column. Epic issues do not belong to scrum boards. Issues returned from this resource include
...
/**
 * Returns the approximate count of all issues from a board, for a given board ID. This is equivalent to counting the
 * issues on all pages returned by [Get issues for board
...
/**
 * Get all issues you have access to that belong to the sprint from the board. Result pagination is token-based, using
 * `nextPageToken` and `maxResults`. This only includes issues that the user has permission to view. Note, if the user
 * does not have permission to view the board, no issues will be returned at all. Issues returned from this resource
