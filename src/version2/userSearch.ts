...
import type { FindBulkAssignableUsersParameters } from './parameters/findBulkAssignableUsersParameters';
import type { FindAssignableUsersParameters } from './parameters/findAssignableUsersParameters';
import type { FindUsersWithAllPermissionsParameters } from './parameters/findUsersWithAllPermissionsParameters';
import type { FindUsersForPickerParameters } from './parameters/findUsersForPickerParameters';
import type { FindUsersParameters } from './parameters/findUsersParameters';
import type { FindUsersByQueryParameters } from './parameters/findUsersByQueryParameters';
import type { FindUserKeysByQueryParameters } from './parameters/findUserKeysByQueryParameters';
import type { FindUsersWithBrowsePermissionParameters } from './parameters/findUsersWithBrowsePermissionParameters';
...
export class UserSearch {
   ...
   *   the projects, use [Get all users](#api-rest-api-2-users-search-get) and filter the records in your code.
   * -
   ...
   *   `projectKeys`.
   */
  async findBulkAssignableUsers(parameters: FindBulkAssignableUsersParameters) {
   ...
   *       in `actionDescriptorId`. You can obtain the IDs of an issue's valid transitions using the `transitions`
   *       option in the `expand` parameter of [ Get issue](#api-rest-api-2-issue-issueIdOrKey-get).
   * -
   ...
   */
  async findAssignableUsers(parameters: FindAssignableUsersParameters) {
    const request: Request = {
   ...
   *       for that project.
   */
  async findUsersWithAllPermissions(parameters: FindUsersWithAllPermissionsParameters) {
    const request: Request = {
   ...
   */
  async findUsersForPicker(parameters: FindUsersForPickerParameters) {
    const request: Request = {
   ...
   * -
   * - **[Permissions](https://developer.atlassian.com/cloud/jira/platform/rest/v2/intro/#permissions) required:** _Browse
   *   users and groups_ [global permission](https://confluence.atlassian.com/x/x4dKLg). Anonymous calls or calls by
   *   users without the required permission return empty search results.
   */
  async findUsers(parameters: FindUsersParameters) {
    const request: Request = {
   ...
   *       if user property `location` is set to value `{"office": {"country": "AU", "city": "Sydney"}}`, then it's
   *       possible to use `[location].office.city is "Sydney"` to match the user.
   * -
   ...
   */
  async findUsersByQuery(parameters: FindUsersByQueryParameters) {
    const request: Request = {
   ...
   * - `is assignee of PROJ AND [propertyKey].entity.property.path is "property value"`
   */
  async findUserKeysByQuery(parameters: FindUserKeysByQueryParameters) {
    const request: Request = {
   ...
   *   users without the required permission return empty search results.
   */
  async findUsersWithBrowsePermission(parameters: FindUsersWithBrowsePermissionParameters) {
    const request: Request = {
