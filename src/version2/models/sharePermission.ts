...
export interface SharePermission {
   ...
   *
   * - `user` Shared with a user. -`group` Shared with a group. If set in a request, then specify `sharePermission.group` as well.
   ...
   *   the `type`. -`project-unknown` Shared with a project that the user does not have access to. Cannot be set in a request.
   */
  type: string;
  project?: Project;
  role?: ProjectRole;
