...
import type { FindUsersAndGroupsParameters } from './parameters/findUsersAndGroupsParameters';
...
export class GroupAndUserPicker {
   ...
   *       their email address in their user profile, partial matches of the email address will not find the user. An
   ...
   *   the item name with the HTML strong tag. Also, each list is wrapped in a response object that contains a header
   *   for use in a picker, specifically _Showing X of Y matching groups_.
   * -
   ...
   */
  async findUsersAndGroups(parameters: FindUsersAndGroupsParameters) {
