...
import type { GetMyPermissionsParameters } from './parameters/getMyPermissionsParameters';
import type { GetBulkPermissionsParameters } from './parameters/getBulkPermissionsParameters';
import type { GetPermittedProjectsParameters } from './parameters/getPermittedProjectsParameters';
...
export class Permissions {
   ...
   *   project, because any user can be a reporter. However, if they are not the user who reported the issue queried
   ...
   *   is granted to Service Project Customer - Portal Access, any users with access to the customer portal will have
   ...
   */
  async getMyPermissions(parameters: GetMyPermissionsParameters) {
   ...
   *   _Administer Jira_ [global permission](https://confluence.atlassian.com/x/x4dKLg) to check the permissions for
   *   other users, otherwise none. However, Connect apps can make a call from the app server to the product to obtain
   ...
   */
  async getBulkPermissions(parameters: GetBulkPermissionsParameters) {
    const request: Request = {
   ...
   * - **[Permissions](https://developer.atlassian.com/cloud/jira/platform/rest/v2/intro/#permissions) required:** None.
   */
  async getPermittedProjects(parameters: GetPermittedProjectsParameters) {
    const request: Request = {
