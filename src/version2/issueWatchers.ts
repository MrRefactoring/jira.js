...
import type { GetIsWatchingIssueBulkParameters } from './parameters/getIsWatchingIssueBulkParameters';
import type { RemoveWatcherParameters } from './parameters/removeWatcherParameters';
import type { GetIssueWatchersParameters } from './parameters/getIssueWatchersParameters';
import type { AddWatcherParameters } from './parameters/addWatcherParameters';
...
export class IssueWatchers {
   ...
   *       to view the issue.
   */
  async getIsWatchingIssueBulk(parameters: GetIsWatchingIssueBulkParameters) {
   ...
   */
  async removeWatcher(parameters: RemoveWatcherParameters) {
    const request: Request = {
   ...
   *       permission](https://confluence.atlassian.com/x/yodKLg) for the project that the issue is in.
   */
  async getIssueWatchers(parameters: GetIssueWatchersParameters) {
    const request: Request = {
      url: `/rest/api/2/issue/${parameters.issueIdOrKey}/watchers`,
      method: 'GET',
    };
   ...
   *       permission](https://confluence.atlassian.com/x/yodKLg) for the project that the issue is in.
   */
  async addWatcher(parameters: AddWatcherParameters) {
    const request: Request = {
      url: `/rest/api/2/issue/${parameters.issueIdOrKey}/watchers`,
      method: 'POST',
    };
