...
import type { GetApplicationPropertyParameters } from './parameters/getApplicationPropertyParameters';
import type { SetApplicationPropertyParameters } from './parameters/setApplicationPropertyParameters';
...
export class JiraSettings {
   ...
   */
  async getApplicationProperty(parameters: GetApplicationPropertyParameters) {
   ...
   * - | `jira.date.picker.java.format` | The date format for the Java (server-side) generated dates. This must be the
   *   same as the `jira.date.picker.javascript.format` format setting. | `d/MMM/yy` |
   * - | `jira.date.picker.javascript.format` | The date format for the JavaScript (client-side) generated dates. This
   *   must be the same as the `jira.date.picker.java.format` format setting. | `%e/%b/%y` |
   * - | `jira.date.time.picker.java.format` | The date format for the Java (server-side) generated date times. This must
   *   be the same as the `jira.date.time.picker.javascript.format` format setting. | `dd/MMM/yy h:mm a` |
   * - | `jira.date.time.picker.javascript.format` | The date format for the JavaScript (client-side) generated date
   *   times. This must be the same as the `jira.date.time.picker.java.format` format setting. | `%e/%b/%y %I:%M %p` |
   * - | `jira.issue.actions.order` | The default order of actions (such as _Comments_ or _Change history_) displayed on
   *   the issue view. | `asc` |
   * - | `jira.view.issue.links.sort.order` | The sort order of the list of issue links on the issue view. | `type,
   *   status, priority` |
   * - | `jira.comment.collapsing.minimum.hidden` | The minimum number of comments required for comment collapsing to
   *   occur. A value of `0` disables comment collapsing. | `4` |
   * - | `jira.newsletter.tip.delay.days` | The number of days before a prompt to sign up to the Jira Insiders newsletter
   ...
   *   format](https://docs.oracle.com/javase/6/docs/api/index.html?java/text/SimpleDateFormat.html). | `h:mm a` |
   * - | `jira.lf.date.day` | The [ day
   *   format](https://docs.oracle.com/javase/6/docs/api/index.html?java/text/SimpleDateFormat.html). | `EEEE h:mm a` |
   * - | `jira.lf.date.complete` | The [ date and time
   *   format](https://docs.oracle.com/javase/6/docs/api/index.html?java/text/SimpleDateFormat.html). | `dd/MMM/yy h:mm
   *   a` |
   * - | `jira.lf.date.dmy` | The [ date
   *   format](https://docs.oracle.com/javase/6/docs/api/index.html?java/text/SimpleDateFormat.html). | `dd/MMM/yy` |
   * - | `jira.date.time.picker.use.iso8061` | When enabled, sets Monday as the first day of the week in the date picker,
   *   as specified by the ISO8601 standard. | `false` |
   ...
   * - | `jira.title` | The text for the application title. The application title can also be set in _General settings_. |
   *   `Jira` |
   * - | `jira.option.globalsharing` | Whether filters and dashboards can be shared with anyone signed into Jira. | `true`
   *   |
   * - | `xflow.product.suggestions.enabled` | Whether to expose product suggestions for other Atlassian products within
   *   Jira. | `true` |
   * -
   * -
   * - #### Other settings
   ...
   *   _Administer Jira_ [global permission](https://confluence.atlassian.com/x/x4dKLg).
   */
  async setApplicationProperty(parameters: SetApplicationPropertyParameters) {
    const request: Request = {
