...
export const FindUsersSchema = z.object({
  /**
   * A query string that is matched against user attributes ( `displayName`, and `emailAddress`) to find relevant
   ...
   * (.) or equals (=) characters cannot be used. The query string cannot be specified using a JSON object. Example:
   * To search for the value of `nested` from `{"something":{"nested":1,"other":2}}` use
