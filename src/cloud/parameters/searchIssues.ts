...
export const SearchIssuesSchema = z.object({
   ...
   *
   * Examples: `"names,changelog"` Returns the display name of each field as well as a list of recent updates to an
   * issue.
   */
  expand: z
    .union([
  ...
  /**
   * Whether to also return issues that belong to [archived
   * projects](https://support.atlassian.com/jira-cloud-administration/docs/archive-a-project/). Issues in archived
   * projects are excluded by default. Setting this to `true` returns them alongside issues from active projects; the
   * _Browse projects_ permission is still required on the archived project. The default is `false`.
   */
  includeArchivedProjects: z.boolean().optional(),
});

export type SearchIssues = z.input<typeof SearchIssuesSchema>;
