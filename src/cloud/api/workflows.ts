import {
  WorkflowHistoryReadResponseDTOSchema,
  type WorkflowHistoryReadResponseDTO,
} from '../models/workflowHistoryReadResponseDTO';
import {
  WorkflowHistoryListResponseDTOSchema,
  type WorkflowHistoryListResponseDTO,
} from '../models/workflowHistoryListResponseDTO';
import {
  WorkflowProjectIssueTypeUsageDTOSchema,
  type WorkflowProjectIssueTypeUsageDTO,
} from '../models/workflowProjectIssueTypeUsageDTO';
import { WorkflowProjectUsageDTOSchema, type WorkflowProjectUsageDTO } from '../models/workflowProjectUsageDTO';
import { WorkflowSchemeUsageDTOSchema, type WorkflowSchemeUsageDTO } from '../models/workflowSchemeUsageDTO';
import { WorkflowReadResponseSchema, type WorkflowReadResponse } from '../models/workflowReadResponse';
import { WorkflowCapabilitiesSchema, type WorkflowCapabilities } from '../models/workflowCapabilities';
import { WorkflowCreateResponseSchema, type WorkflowCreateResponse } from '../models/workflowCreateResponse';
import {
  WorkflowValidationErrorListSchema,
  type WorkflowValidationErrorList,
} from '../models/workflowValidationErrorList';
import {
  DefaultWorkflowEditorResponseSchema,
  type DefaultWorkflowEditorResponse,
} from '../models/defaultWorkflowEditorResponse';
import { WorkflowPreviewResponseSchema, type WorkflowPreviewResponse } from '../models/workflowPreviewResponse';
import { WorkflowSearchResponseSchema, type WorkflowSearchResponse } from '../models/workflowSearchResponse';
import { WorkflowUpdateResponseSchema, type WorkflowUpdateResponse } from '../models/workflowUpdateResponse';
import type { ReadWorkflowFromHistory } from '../parameters/readWorkflowFromHistory';
import type { ListWorkflowHistory } from '../parameters/listWorkflowHistory';
import type { DeleteInactiveWorkflow } from '../parameters/deleteInactiveWorkflow';
import type { GetWorkflowProjectIssueTypeUsages } from '../parameters/getWorkflowProjectIssueTypeUsages';
 ...
 *
 * **Note:** Stored workflow data expires after 60 days. Additionally, no data from before the 30th of October 2025 is
 * available.
 *
 ...
 * type ID pair. The response includes the scope of the workflow, defined as global/project-based, and a list of project
 * types that the workflow is scoped to. It also includes all rules organised into their broad categories (conditions,
 * validators, actions, triggers, screens) as well as the source location (Atlassian-provided, Connect, Forge).
 *
 * **[Permissions](https://developer.atlassian.com/cloud/jira/platform/rest/v3/intro#permissions) required:**
 *
 ...
 *
 * A validation that ensures a specific field's value meets the defined criteria before allowing an issue to transition
 * in the workflow.
 *
 ...
 *   doesn't check a required field if its context isn't configured for the current issue. When set to `false`, the
 *   validator requires a field even if its context is invalid. Allowed values: `true`, `false`.
 * - `errorMessage` is the error message to display if the user does not provide a value during the transition. A default
 ...
 * ##### Check field value
 *
 * A condition rule evaluates as true if a specific field's value meets the defined criteria. This rule ensures that an
 * issue can only transition to the next step in the workflow if the field's value matches the desired condition.
 *
 *     {
 ...
 * permissions, maintaining control over who can transition an issue. This condition evaluates as `true` if any of the
 ...
 * - `mostRecentStatusOnly` when true only considers the most recent status for the condition evaluation. Allowed values:
 *   `true`, `false`.
 ...
 * ##### Trigger agent
 *
 * A post function that triggers a Jira AI agent for the issue after the transition runs, using the configured agent and
 * an optional prompt. The agent run is asynchronous: it is scheduled after the transition finishes and does not block
 * the transition.
 *
 *     {
 ...
 * - `promptValue` optional text passed to the agent as a user prompt after the transition runs; use an empty string if no
 ...
 * and project workflows. If workflow names are specified in the query string, details of those workflows are returned.
