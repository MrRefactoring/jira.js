...
export const WorkflowSchemeSchema = apiObject({
   ...
   * The default workflow has _All Unassigned Issue Types_ assigned to it in Jira.
   */
  originalDefaultWorkflow: z.string().optional(),
  /**
   * For draft workflow schemes, this property is the issue type to workflow mappings for the original workflow
   * scheme, where each mapping is an issue type ID and workflow name pair. Note that an issue type can only be mapped
   ...
   * works:
   *
   * - Update an active workflow scheme with `updateDraftIfNeeded` set to `true`: If a draft workflow scheme exists, it
   *   is updated. Otherwise, a draft workflow scheme is created.
