...
export const IssueBulkMovePayloadSchema = apiObject({
   ...
   * The format is `<project ID or key>,<issueType ID>,<parent ID or key>`. It should be unique across mappings
   * provided in the payload. If you provide multiple mappings for the same key, only one will be processed. However,
   * the operation won't fail, so the error may be hard to track down.
   *
   * - _**Destination project**_ (Required): ID or key of the project to which the issues are being moved.
   * - _**Destination issueType**_ (Required): ID of the issueType to which the issues are being moved.
