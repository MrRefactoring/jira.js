...
export const JiraExpressionComplexitySchema = apiObject({
   ...
   *
   * - `issues.map(i => i.comments)` performs as many expensive operations as there are issues on the issues list. So
   *   this parameter returns `N`, where `N` is the size of issue list.
   * - `new Issue(10010).comments` gets comments for one issue, so its complexity is `2` (`1` to retrieve issue 10010
   *   from the database plus `1` to get its comments).
   */
