...
export async function getEvents(client: Client, parameters: GetEvents, options?: RequestOptions): Promise<EventPage> {
  const config: SendRequestOptions<EventPage> = {
    ...
    searchParams: {
      ...
      actor: Array.isArray(parameters.actor) ? parameters.actor.join(',') : parameters.actor,
      ip: Array.isArray(parameters.ip) ? parameters.ip.join(',') : parameters.ip,
      product: Array.isArray(parameters.product) ? parameters.product.join(',') : parameters.product,
