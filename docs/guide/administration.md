...
One more distinction, measured rather than documented: `userManagement` refuses a key created *with* scopes — every
operation answers `403` naming a `manage:org` scope that the key creation flow does not offer. Create that key without
scopes. The scoped kind reaches `admin` fine.
