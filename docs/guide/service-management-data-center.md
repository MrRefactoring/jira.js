...
The repository carries a throwaway Service Management Data Center alongside the Jira Software one, and both run at
once — Jira Software on 8080, Service Management on 8081:

```bash
pnpm jsm-dc:up     # start it, run the setup wizard, seed a service desk
pnpm test:live:jsm # run the Service Management suites against it
pnpm jsm-dc:down   # stop it and delete its data
```

It is a sibling of the Data Center rig rather than a replacement: `atlassian/jira-software` carries neither Service
Management nor Assets, and the platform suites need boards and sprints this image does not have. It runs 10.3 for
the reason the [Data Center guide](./data-center) gives for its own pin, and it needs a Service Management timebomb
licence rather than a Jira Software one — under the latter every `/rest/servicedeskapi` endpoint answers 403, while
`/rest/assets/1.0` answers in full. Service Management indexes more than Jira Software does, so its container is
given 2 GB of heap, below which a cold start thrashes.
