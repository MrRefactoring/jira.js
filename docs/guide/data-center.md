...
Two environment variables in `docker/jira-dc/compose.yaml` are load-bearing if you adapt it. `JDBC_DRIVER` has to be
set, or the generated `dbconfig.xml` carries an empty `<driver-class>`, Jira cannot open the connection, and the
wizard asks for the database after all — with every field blank, which reads as the variables having been ignored
rather than incomplete. `postgres72` is Atlassian's name for any PostgreSQL, whatever version the server actually
runs. Only the database step is driven by environment at all: licence, administrator and mail have no variables and
are driven over HTTP by `scripts/jiraDc.ts`.
