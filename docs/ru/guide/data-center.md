...
Две переменные окружения в `docker/jira-dc/compose.yaml` несущие, если вы будете его править. `JDBC_DRIVER` обязана
...
а не «неполны». `postgres72` — это имя, которым Atlassian называет любой PostgreSQL, независимо от версии сервера.
Переменными управляется только шаг с базой: лицензия, администратор и почта переменных не имеют и настраиваются по
HTTP из `scripts/jiraDc.ts`.
