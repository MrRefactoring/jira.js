...
Ещё одно различие, измеренное, а не вычитанное: `userManagement` отвергает ключ, созданный *со scope* — каждая
операция отвечает `403` и называет scope `manage:org`, которого в форме создания ключа нет. Такой ключ создавайте без
scope. Для `admin` scoped-ключ подходит.
