...
> Эта страница — про **Jira Cloud**. Self-hosted инстанс сам себе сервер авторизации: резолвить `cloudId`
> не нужно, и ничего не идёт через `auth.atlassian.com`. Про этот поток — в
> [Jira Data Center](./data-center#oauth-2-0).
