...
В репозитории есть одноразовый Service Management Data Center рядом с инстансом Jira Software, и они поднимаются
одновременно — Jira Software на 8080, Service Management на 8081:

```bash
pnpm jsm-dc:up     # поднять, пройти мастер установки, создать сервис-деск
pnpm test:live:jsm # прогнать сюиты Service Management против него
pnpm jsm-dc:down   # остановить и удалить данные
```

Это сосед стенда Data Center, а не замена ему: в `atlassian/jira-software` нет ни Service Management, ни Assets, а
платформенным сюитам нужны доски и спринты, которых в этом образе нет. Версия 10.3 — по той же причине, которую
[руководство по Data Center](./data-center) приводит для своего пина. Лицензия нужна именно Service Management, а не
Jira Software: под второй каждый эндпоинт `/rest/servicedeskapi` отвечает 403, тогда как `/rest/assets/1.0`
отвечает полностью. Service Management индексирует больше, чем Jira Software, поэтому его контейнеру отдано 2 ГБ
кучи — ниже холодный старт начинает буксовать.
