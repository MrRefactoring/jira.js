...
// Vitest's `test.env` (loaded via `loadEnv()` in `vitest.config.ts`) is injected into
// worker `process.env` at runtime, but `globalSetup` runs in the main process before that.
// Mirror the same load here so we see JIRA_* variables that only live in the .env file.
...
function extractCliFilters(): string[] {
  // Vitest positional args (filters) come after the 'run' / 'watch' subcommand and any flags.
  // `pnpm exec vitest run tests/unit` → argv filters = ['tests/unit']
  // `pnpm exec vitest run --coverage` → argv filters = []
