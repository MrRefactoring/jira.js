import { JiraExpressionsAnalysisSchema, type JiraExpressionsAnalysis } from '#/models/jiraExpressionsAnalysis';
import {
  JExpEvaluateJiraExpressionResultSchema,
...
/**
 * Evaluates a Jira expression and returns its value. The difference between this and `eval` is that this endpoint uses
 * the enhanced search API when evaluating JQL queries. This API is eventually consistent, unlike the strongly
 ...
 * #### Context variables
 *
 * The following context variables are available to Jira expressions evaluated by this resource. Their presence depends
 * on various factors; usually you need to manually request them in the context object sent in the payload, but some of
 ...
 *   app](https://developer.atlassian.com/cloud/jira/platform/index/#connect-apps) that made the request. Available only
 *   for authenticated requests made by Connect apps (read more here: [Authentication for Connect
 ...
 *   [Issues](https://developer.atlassian.com/cloud/jira/platform/jira-expressions-type-reference#issue)): A collection
 *   of issues matching a JQL query. Available only when JQL is provided in the request context object.
 ...
 * - `sprint` ([Sprint](https://developer.atlassian.com/cloud/jira/platform/jira-expressions-type-reference#sprint)): The
 *   current sprint. Available only when the sprint is provided in the request context object.
 ...
 *
 * - `user`: A [user](https://developer.atlassian.com/cloud/jira/platform/jira-expressions-type-reference#user) specified
 *   as an Atlassian account ID.
 ...
 *
 * **[Permissions](https://developer.atlassian.com/cloud/jira/platform/rest/v3/intro#permissions) required**: None.
 * However, an expression may return different results for different users depending on their permissions. For example,
 * different users may see different comments on the same issue.\
