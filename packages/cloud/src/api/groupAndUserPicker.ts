 ...
 *   - Issue types.
 *
 *   If multiple projects or issue types are specified, they must be a subset of those enabled for the custom field or no
 *   results are returned. For example, if a field is enabled for projects A, B, and C then the search could be limited
 *   to projects B and C. However, if the search is limited to projects B and D, nothing is returned.
 ...
 * end, the returned object includes an `html` field for each list. This field highlights the matched query term in the
 * item name with the HTML strong tag. Also, each list is wrapped in a response object that contains a header for use in
 * a picker, specifically _Showing X of Y matching groups_.
 *
