import { PermissionsSchema, type Permissions } from '#/models/permissions';
import { BulkPermissionGrantsSchema, type BulkPermissionGrants } from '#/models/bulkPermissionGrants';
import { PermittedProjectsSchema, type PermittedProjects } from '#/models/permittedProjects';
 ...
 * project context but may not have the permission for any or all issues. For example, if Reporters have the EDIT_ISSUES
 * permission a user would be shown as having this permission in the global context or the context of a project, because
 * any user can be a reporter. However, if they are not the user who reported the issue queried they would not have
 * EDIT_ISSUES permission for that issue.
 *
 ...
 * this will be evaluated similarly to a user in the customer portal. For example, if the BROWSE_PROJECTS permission is
 * granted to Service Project Customer - Portal Access, any users with access to the customer portal will have the
 ...
 * users, otherwise none. However, Connect apps can make a call from the app server to the product to obtain permission
