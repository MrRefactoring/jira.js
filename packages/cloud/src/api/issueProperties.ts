...
import { type BulkSetIssuesPropertiesList } from '#/parameters/bulkSetIssuesPropertiesList';
import { type BulkSetIssuePropertiesByIssue } from '#/parameters/bulkSetIssuePropertiesByIssue';
import { type BulkSetIssueProperty } from '#/parameters/bulkSetIssueProperty';
import { type BulkDeleteIssueProperty } from '#/parameters/bulkDeleteIssueProperty';
 ...
 *   in the response to determine the status of the task and use [Get
 ...
 *
 * - _Browse projects_ and _Edit issues_ [project permissions](https://confluence.atlassian.com/x/yodKLg) for the project
 ...
 * [restrictions](https://developer.atlassian.com/cloud/jira/platform/jira-expressions/#restrictions) that apply to all
 * Jira expressions.
 *
 ...
 *
 * If more than one criteria is specified, they are joined with the logical _AND_: only issues that satisfy all criteria
 * are eligible.
 *
 * If an invalid combination of criteria is provided, an error is returned. For example, specifying a `currentValue` and
 * `hasProperty` as _false_ would not match any issues (because without the property the property cannot have a value).
 *
 * The filter is optional. Without the filter all the issues visible to the user and where the user has the EDIT_ISSUES
 ...
 *
 * **[Permissions](https://developer.atlassian.com/cloud/jira/platform/rest/v3/intro#permissions) required:** Property
