...
// These tests stub the global `fetch` and use fake timers. The unit suite runs with `--sequence.concurrent`,
// so everything is wrapped in a single sequential suite (and the tests themselves are sequential) to
// serialize access to the stubbed global across describe blocks.
...
describe.sequential('OAuth2Manager', () => {
  describe('proactive refresh', () => {
    test('refreshes when the token is within the skew window of expiry', async () => {
      const fetchStub = stubFetch().resolves(jsonResponse(tokenData()));
  ...
  describe('rotation & persistence', () => {
    test('rotates the refresh token and persists via onTokenRefresh', async () => {
      ...
      fetchStub.onFirstCall().resolves(jsonResponse(tokenData({ refresh_token: 'r2' })));
      fetchStub.onSecondCall().resolves(jsonResponse(tokenData({ refresh_token: 'r3' })));
      ...
      expect(requestBody(fetchStub.getCall(1))).toMatchObject({ grant_type: 'refresh_token', refresh_token: 'r2' });
    });
  ...
  describe('cloudId resolution', () => {
    ...
    test('resolves and caches the cloudId from accessible-resources', async () => {
      const fetchStub = stubFetch().resolves(
        jsonResponse([{ id: 'cid-1', name: 'S', url: 'https://s.atlassian.net', scopes: [], avatarUrl: '' }]),
      );
      const manager = new OAuth2Manager({ accessToken: 'a' });

      expect(await manager.getBaseUrl()).toBe('https://api.atlassian.com/ex/jira/cid-1');
      expect(await manager.getBaseUrl()).toBe('https://api.atlassian.com/ex/jira/cid-1');
      expect(fetchStub.calledOnce).toBe(true); // cached after the first resolve
    });
    ...
    test('throws when no resources are accessible', async () => {
      stubFetch().resolves(jsonResponse([]));
      const manager = new OAuth2Manager({ accessToken: 'a' });
    ...
    test('throws when siteUrl does not match any resource', async () => {
      stubFetch().resolves(
        jsonResponse([{ id: 'cid-a', name: 'A', url: 'https://a.atlassian.net', scopes: [], avatarUrl: '' }]),
      );
