...
describe('assets schemas and types', () => {
  ...
  it('creates, updates and deletes a schema', async () => {
    ...
    try {
      ...
      const deleted = await api.objectSchemas.deleteSchema({ id: String(created.id) });

      expect(deleted.id).toBe(created.id);
    } finally {
      await api.objectSchemas.deleteSchema({ id: String(created.id) }).catch(() => {});
    }
  });
  ...
  it('creates, updates, repositions and deletes an object type', async () => {
    ...
    try {
      ...
      const deleted = await api.objectTypes.deleteObjectType({ id: String(created.id) });

      expect(deleted.id).toBe(created.id);
    } finally {
      await api.objectTypes.deleteObjectType({ id: String(created.id) }).catch(() => {});
    }
  });
  ...
  it('creates, updates and deletes an attribute on an object type', async () => {
    ...
    try {
      ...
      await api.objectTypeAttributes.deleteObjectTypeAttribute({ id: String(created.id) }).catch(() => {});
    }
  });
});
