...
describe('assets content', () => {
  ...
  it('attaches a file to an object, lists it and removes it', async () => {
    ...
    const removed = await api.attachments.deleteAttachment({ attachmentId: String(attachment!.id) });
  ...
  afterAll(async () => {
    ...
    await api.attachments.deleteAttachment({ attachmentId: String(attachmentId) }).catch(() => {});
  });
