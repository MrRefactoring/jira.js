 ...
 * list, and on a site with no Service Management at all it refuses the lookup outright. Either way there is no
 * workspace to point a client at — so this stands down, visibly, rather than failing sixty times over a plan. That is the shape `tests/live/setup/entitlement.ts` established for the Cloud suites, and for the same
