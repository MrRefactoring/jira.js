...
/**
 * Live suite for the `status` API (`searchStatuses`, `getStatusesById`, `getStatusesByName`, the usage reads, and the
...
describe('Jira Cloud — status (live, read-only)', () => {
  ...
  beforeAll(async () => {
    ...
    const page = await client.status.searchStatuses({ maxResults: 5 }).catch(() => undefined);
  ...
  it('searches statuses, or refuses typed without admin rights', async () => {
    if (!permitted) {
      const error = await client.status.searchStatuses({}).catch((e: unknown) => e);
    ...
    const page = await client.status.searchStatuses({ maxResults: 5 });
  ...
  it('returns a different set of statuses from the older API', async () => {
    ...
    const modern = await client.status.searchStatuses({ maxResults: 100 });
  ...
  it('filters the search by category and by name', async () => {
    ...
    const done = await client.status.searchStatuses({ statusCategory: 'DONE', maxResults: 50 });

    for (const status of done.values ?? []) expect(status.statusCategory).toBe('DONE');

    const all = await client.status.searchStatuses({ maxResults: 1 });
    const name = all.values?.[0]?.name;

    if (!name) return;

    const byName = await client.status.searchStatuses({ searchString: name, maxResults: 50 });
  ...
  it('distinguishes global statuses from project-scoped ones', async () => {
    ...
    const scoped = await client.status.searchStatuses({ projectId: project.id!, maxResults: 50 }).catch(() => undefined);
